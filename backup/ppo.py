...
class Critic(nn.Module):
    def __init__(self, num_state, hidden_size):
        ...
        self.fc_stack = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_size, hidden_size),
            nn.Tanh(),  # be cautious if using tanh
...
class PPO():
    ...
    def __init__(self, env_name):
        ...
        # self.actor_optim = optim.Adam(self.actor.parameters(), 1e-3)
        # self.critic_optim = optim.Adam(self.critic.parameters(), 1e-3)
        self.optim = optim.Adam(itertools.chain(self.actor.parameters(), self.critic.parameters()), lr = 1e-3)
    ...
    def update(self, i_epoch):
        ...
        for t in reversed(range(self.T_timesteps)):
            ...
            G[t] = stat.reward[t] + self.gamma * prev_r * stat.mask[t]
        ...
        G = normclip(G) # since we're using tanh, so constrain critic's output to smaller range
        ...
        for _ in range(self.K_epochs * int(self.T_timesteps / self.M_minibatchsize)):
            ...
            for i in range(0, self.T_timesteps, self.M_minibatchsize):
                ...
                L_VF = F.mse_loss(value_from_critic, value_from_statistics)
